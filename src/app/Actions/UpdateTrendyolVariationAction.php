...
class UpdateTrendyolVariationAction
{
    public function __construct(
        public SeedVariationsForTrendyolAction $seedVariationsForTrendyolAction,
    )
    {
    }
    ...
    public function execute(array $data): void
    {
        foreach ($data as $result) {
            if (!$result['success']) {
                ...
                $this->seedVariationsForTrendyolAction->execute($result, $result['sync'] ?? false);
            }
    ...
    private function logError($result): void
    {
        ...
        LogManager::logProduct($product, 'trendyol-sync-error', [
            'result' => $result,
        ]);
    }
}
